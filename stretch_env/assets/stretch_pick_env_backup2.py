...
class StretchPickEnv(MujocoEnv, utils.EzPickle):
    ...
    def reset_model(self):
        ...
        TABLE_X_CENTER = 0.0
        TABLE_Y_CENTER = -0.7
        TABLE_X_HALF   = 0.3   # < 0.64, leave margin
    ...
    def step(self, a):
        ...
        # Normal action: normalized absolute position in [-1, 1] per actuator.
        # a[i] = -1 maps to low[i], a[i] = +1 maps to high[i].
        # The policy outputs a target position directly — no integration.
        ...
        a = np.clip(np.copy(a), -1.0, 1.0)
        a[3] = 1.0  # keep gripper fully open during collection
        ...
        # De-normalize: map [-1, 1] → [low, high]
        self._ctrl_target = low + (a + 1.0) / 2.0 * (high - low)

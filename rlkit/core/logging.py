...
class Logger(object):
    ...
    def add_tensorboard_output(self, file_name):
        from rlkit.experimental.kuanfang import tensorboard_logger
    ...
    def record_tabular(self, key, val):
        ...
        if self._use_tensorboard:
            self.tensorboard_logger.log_value(
                self._tabular_prefix_str + str(key), val, self.epoch)
        ...
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                self._save_params_to_file(
                    params, file_name, mode=self._save_param_mode)
...
def create_log_dir(
    ...
    if run_id is not None:
        ...
        if variant.get("num_exps_per_instance", 0) > 1:
            ...
            trial_name = "%s/run%s/id%s/%s--s%d" % (
                exp_name, run_id, exp_id, timestamp, seed)
        else:
            trial_name = "{}/run{}/id{}".format(exp_name, run_id, exp_id)

    else:
        trial_name = create_trial_name(
            exp_name,
            exp_id=exp_id,
            seed=seed,
            add_time_suffix=add_time_suffix)
    ...
    if include_exp_name_sub_dir:
        log_dir = osp.join(
            base_log_dir, exp_name.replace("_", "-"), trial_name)
    else:
